  ...
  return fetch(URL)
    ...
    .then(body => {
      ...
      $('.news-card').each((i, element) => {
        ...
        const imageUri = imageUriString.substring(
          imageUriString.indexOf('http'),
          imageUriString.length - 3,
        );

        const divCardFooter = $element.find(
          'div.news-card-footer.news-right-box',
        );
